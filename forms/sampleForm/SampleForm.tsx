...
import config from "./sampleFormTabInfo";
import Validator from "./sampleFormValidator";
...
//Tab Components
import TabSampleCase from "./tabSampleCase";
...
class SampleForm extends React.Component<Props, State> {
  state = {
  ...
  formPMListener = (e: any) => {
    try {
      ...
      if (data.activeTab) {
        ...
        } else if (data.formAction === "mannualUpdate") {
          console.log("mannualUpdate in Sample Form", data.streamData);
          let {
            variableValueInfo,
            variableNameInfo: { variableName }
          } = data.streamData;
          this.props.change(variableName, variableValueInfo);
          // this.props.change(data.streamData.parserVar, {
          //   type: "string",
          //   value: data.streamData.parserValue,
          //   valueInfo: {}
          // });
        }
        // }
      ...
      if (this.props.env === "OUTSIDE") {
        ...
        Object.keys(values).forEach(item => {
          if (values[item] && values[item].type === "file") {
            ...
            //skipp the existing file variable
            //existing files from taskVariables
            // variables[item] = values[item]
          }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        >
          {this.state.activeTab === "sampleForm" && (
            <TabSampleCase
...
let SampleFormReduxFormWrapper = reduxForm({
  //pass your form key
  form: "sampleForm",
  validate: Validator,
...
const mapStateToProps = (state, props) => {
  ...
  return {
    ...
    isPristine: isPristine("sampleForm")(state),
